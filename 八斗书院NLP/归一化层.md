...
![2025-09-09_10-11.jpg](https://cdn.jsdelivr.net/gh/zilong-ding/note-gen-image-sync@main/ec674d5a-0e93-4a8e-bffc-a1d10cdac8df.jpeg)
...
$$
\begin{aligned}&y=\frac{x-\mathbf{E}[x]}{\sqrt{\mathbf{Var}[x]+\epsilon}}*\gamma+\beta\\&\text{其中前一项是归一化过程。分母中的 }\epsilon\text{ 是一个非常小的数,作用是防止数值计算不稳定。}\gamma\\&\text{和 }\beta\text{ 是仿射参数,将归一化后的数据再次放缩得到新的数据,}\gamma\text{可以理解为标准差,}\beta\text{可以}\\&\text{理解为均值,它们两个一般是可学习的。}\end{aligned}
$$
...
#### 📈 6. 在目标检测/分割中的优势（Mask R-CNN 实验）

何恺明在论文中重点验证了 GN 在 **Mask R-CNN（小 batch=1\~2）** 上的表现：

* BatchNorm（BN）：batch=2 时 mAP 下降明显
* GroupNorm（GN）：batch=1 或 2 时，mAP 稳定，甚至略超 BN（batch=16）
* 训练更稳定，收敛更快

📌 **为什么？**

* 检测/分割中，每张图像内容差异大 → BatchNorm 的跨样本统计“互相污染”
* GroupNorm 只用当前图像 → 无干扰，统计纯净
* 同时保留组内通道信息 → 不像 InstanceNorm 那样“太局部”
