...
| 维度           | Post-LN（原始Transformer)     | Pre-LN (现代主流)             | DeepNorm (极深网络)                | RMSNorm (高效归一化)                  |
| -------------- | ----------------------------- | ----------------------------- | ---------------------------------- | ------------------------------------- |
| 全称           | Post-Layer Normalization      | Pre-Layer Normalization       | Deep Normalization                 | Root Mean Square Layer Normalization  |
| 核心思想       | 残差连接后做LayerNorm         | 子层输入前做 LayerNorm        | 缩放残差+初始化控制，稳 定极深网络 | 简化LayerNorm：不减均值， 只除RMS     |
| 是否减均值     | 是                            | 是                            | 是（仍用LN）                       | 否                                    |
| 是否有β偏移   | 是                            | 是                            | 是                                 | ×通常省略                            |
| 训练稳定性     | ×差 (浅层梯度小，输出易爆炸) | 好（梯度均衡，输出稳 定)      | 极好(支持1000+层)                  | 与LN相当或略优                        |
| 是否需要warmup | 必需(否则训练崩溃)            | ×通常不需要                  | ×不需要                           | ×不需要                              |
| 收敛速度       | 慢（依赖warmup）              | 快                            | 中等 (需配合初始化)                | 快 (计算更简单)                       |
| 计算效率       | 中等                          | 中等                          | 中等                               | 更高（少一次减均值，内存 访问少)      |
| 最终性能       | 调优后略高 (理论上限)         | 略低或相当 (但更鲁棒)         | 超深模型中显著更高                 | 与LN相当，部分任务略优                |
| 适用模型深度   | <32层（更深需warmup+调参)     | 12～128层(工业主流)           | 200~1000+层（学术探 索)            | 任意(作为LN替代)                      |
| 工程复杂度     | 低 (原生支持)                 | 低 (原生支持) LLaMA、GPT-3、  | 高 (需改残差+初始化)               | 低（易实现）                          |
| 主流模型采用   | BERT、T5、原始 Transformer    | Mistral、Gemma、 Qwen、Falcon | ×无 (学术研究为主)                | LLaMA、Mistral、 Gemma、Phi、StableLM |
| 推荐使用场景   | 复现原始论文、有调参能力      | 99%新项目首选                 | 超深模型研究(>200层)               | 追求训练/推理效率，替代 LayerNorm     |
| 可组合性       | 可+DeepNorm(用于深模型)       | 可+RMSNorm (LLaMA组合)        | 理论可+RMSNorm（未验 证)           | 常与Pre-LN 组合使用                   |
